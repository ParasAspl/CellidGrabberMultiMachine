    ...
    public partial class Dashboard : MetroForm
    {
        ...
        private  void scan2GNetwork()
        {
            try
            {
                ...
                serialWrite("AT+CNBP=0xFFFFFFFF7FFFFFFF,0x000007FF03DF3FFF,0x000000000000003F");
                 ...
                 serialWrite("AT+CNMP=13"); waitForOutput(2);
                serialWrite("AT+CMSSN"); waitForOutput(2);
                serialWrite("AT+CSURV"); waitForOutput(10);
                serialWrite("AT+CSURV"); waitForOutput(2);
        ...
        private void scan3GNetwork()
        {
            try
            {
                ...
                len = 11;
                outputFile = @"C:\cell id driver\amar\amar\GodSharpDemo\2goutput.txt";
                serialWrite("AT+CNBP=0xFFFFFFFF7FFFFFFF,0x000007FF03DF3FFF,0x000000000000003F"); Thread.Sleep(50);
                serialWrite("AT+CNMP=14"); waitForOutput(2);
                serialWrite("AT+CMSSN"); waitForOutput(2);
                serialWrite("AT+CLUARFCN"); waitForOutput(5);
                serialWrite("AT+CLUCELL"); waitForOutput(10);
                serialWrite("AT+CSNINFO?"); waitForOutput(2);
                serialWrite("AT+CSNINFO?"); waitForOutput(2);
                serialWrite("AT+CCINFO"); waitForOutput(2);
                serialWrite("AT+CCINFO"); waitForOutput(2);
            }
            catch (Exception ex)
            {
        ...
        private void  scan4GNetwork()
        {
            try
            {
                ...
                waitForOutput(2);
                 serialWrite("AT+CMSSN"); waitForOutput(5);
                ...
                for (int i = 0; i < bandS.Length; i++)
                {
                    ...
                    serialWrite("AT+CNBP=," + bandS[i]); Thread.Sleep(2);

                    serialWrite("AT+CSNINFO?"); waitForOutput(2);

                    serialWrite("AT+CMGRMI=4"); waitForOutput(2);
                }
            }
            catch (Exception ex)
            {
...
serialWrite("AT+CNBP=0xFFFFFFFF7FFFFFFF,0x000007FF03DF3FFF,0x000000000000003F"); Thread.Sleep(2000);
        ...
        private void serialPort2_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if ((((dataRec == null) || (count2G >= 5 && a != "ALL") || (count3G >= 3 && a != "ALL") || (count4G >= 5 && a != "ALL"))))
            {
            ...
            else
            {
                ...
                if ((!(dataRec.Contains("CCINFO") || dataRec.Contains("ccinfo"))) && ((dataRec.Contains("arfcn:")
                        && dataRec.Contains("mcc:")) || (dataRec.Contains("ARFCN:") && dataRec.Contains("MCC:")))  || a == "ALL"))
                {
                    net = "2G";
        ...
        public List<Dictionary<string, string>> clean(String[] lines)
        {
            ...
            {
                //waitfor output -Clean if not output then clear
                //queue remove
