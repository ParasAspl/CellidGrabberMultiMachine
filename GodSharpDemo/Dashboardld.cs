...
namespace GodSharpDemo
{
        ...
        private async void btnStart_ClickAsync(object sender, EventArgs e)
        {
            ...
            if (DdlMode.SelectedItem != null && metroComboBox1.SelectedItem != null)
            {
                ...
                if (serialPort2.IsOpen)
                {
                    ...
                    {
                        case "2G":await scan2GNetwork(); break;
                        case "3G":await scan3GNetwork(); break;
                        case "4G":await scan4GNetwork(); break;
    ...
    //.Subscribe(_ => { Console.WriteLine("All done"); },
        ...
        private async Task scan2GNetwork()
        {
            try
            {
                ...
                await Task.Run(() => serialWrite("AT+CNMP=13"));
                await Task.Run(() => serialWrite("AT+CMSSN"));
                await Task.Run(() => serialWrite("AT+CSURV"));
        ...
        private async Task scan3GNetwork()
        {
            try
            {
                net = "3G";
                len = 11;
                outputFile = @"C:\cell id driver\amar\amar\GodSharpDemo\2goutput.txt";
                await Task.Run(() => serialWrite("AT+CNBP=0xFFFFFFFF7FFFFFFF,0x000007FF03DF3FFF,0x000000000000003F"));
                await Task.Run(() => serialWrite("AT+CNMP=14"));
                await Task.Run(() => serialWrite("AT+CMSSN"));
                await Task.Run(() => serialWrite("AT+CLUARFCN"));
                await Task.Run(() => serialWrite("AT+CLUCELL"));
                await Task.Run(() => serialWrite("AT+CSNINFO?"));
                await Task.Run(() => serialWrite("AT+CSNINFO?"));
                await Task.Run(() => serialWrite("AT+CCINFO"));
                await Task.Run(() => serialWrite("AT+CCINFO"));
        ...
        private async Task scan4GNetwork()
        {
            try
            {
                net = "4G";
                len = 15;
                outputFile = @"C:\cell id driver\amar\amar\GodSharpDemo\2goutput.txt";
                await Task.Run(() => serialWrite("AT+CNMP=38"));
                await Task.Run(() => serialWrite("AT+CMSSN"));
                ...
                for (int i = 0; i < bandS.Length; i++)
                {
                    ...
                    await Task.Run(() => serialWrite("AT+CNBP=," + bandS[i]));

                    await Task.Run(() => serialWrite("AT+CSNINFO?"));

                    await Task.Run(() => serialWrite("AT+CMGRMI=4"));
...
serialWrite("AT+CNBP=0xFFFFFFFF7FFFFFFF,0x000007FF03DF3FFF,0x000000000000003F"); Thread.Sleep(2000);
        ...
        private void serialPort2_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if ((((dataRec == null) || (count2G >= 8 && net != "ALL") || (count3G >= 2 && net != "ALL") || (count4G >= 5 && net != "ALL"))))
            {
        ...
        public Dictionary<String, string> dataCleaner(String data)
        {
            ...
            if (data.ToUpper().Contains("+CCINFO:"))
            {
                info = "true";
                typ = "ccin";
            }
            if (data.ToUpper().Contains("+CMGRMI:"))
            {
                typ = "cmg";
                len = 15;
            }
