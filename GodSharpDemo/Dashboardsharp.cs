    ...
    public partial class Dashboard : MetroForm
    {
        ...
        public bool establishConnection()
        {
            try
            {
                ...
                //SerialPort port = new SerialPort(globalPortName.Trim());
                GodSerialPort gsp = new GodSerialPort(globalPortName, 9600, 0);
                gsp.UseDataReceived(true, (sp, bytes) => {
                    getRegion();
                   // string buffer = string.Join(" ", bytes);
                    string buffer = string.Join(" ", Encoding.ASCII.GetString(bytes));
                   // gsp.WriteLine("receive data:" + buffer);
                  //  gsp.WriteAsciiString(buffer);
               ...
               //   var buffer = new StringBuilder();
                var commandsToSend = new string[] { "AT+CNBP=0xFFFFFFFF7FFFFFFF,0x000007FF03DF3FFF,0x000000000000003F",
                        "AT+CNMP=13",
                        "AT+CMSSN" ,
                    "AT+CCINFO" };
            ...
            {
               // MessageBox.Show(port ? "Successfully connected" : "Not connected");
            }
